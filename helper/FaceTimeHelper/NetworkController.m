...
- (void)connect {
    ...
    if (![asyncSocket connectToHost:@"localhost" onPort:port error:&err]) {
        ...
        DLog("FACETIMEHELPER: Error connecting to socket: %{public}@", err);
    }
...
- (void)socketDidSecure:(GCDAsyncSocket *)sock {
    DLog("FACETIMEHELPER: socketDidSecure:%{public}p", sock);
}

// called when a message is sent to the server
- (void)socket:(GCDAsyncSocket *)sock didWriteDataWithTag:(long)tag {
    DLog("FACETIMEHELPER: socket:%p didWriteDataWithTag:%ld", sock, tag);
}

- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag {
    NSString* str = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    DLog("FACETIMEHELPER: event: %{public}@ socket:%{public}p didReadData:withTag:%{public}ld", str, sock, tag);
...
- (void)socketDidDisconnect:(GCDAsyncSocket *)sock withError:(NSError *)err {
    DLog("FACETIMEHELPER: Disconnected from server, attempting to reconnect in 5 seconds...");
    double delayInSeconds = 5.0;
    dispatch_time_t popTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(delayInSeconds * NSEC_PER_SEC));
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        DLog("FACETIMEHELPER: Attempting to reconnect");
        [self connect];
    });
}
